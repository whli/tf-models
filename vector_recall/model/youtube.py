...
def dnn_layer(features):
    ...
    with tf.variable_scope("Embedding-layer", reuse=tf.AUTO_REUSE):
        ...
        feat_vals = tf.reshape(feat_vals, shape=[-1, common_field_size, 1])     # reshape for_warn
...
def main(_):
    ...
    test_data = "/mnt1/local_v1/vector_recall/train_data/libtest.tfrecord"
    te_files = [test_data]
    ...
    labels = tf.reshape(labels,shape=[-1,1])    # reshape  for_warn
    ...
    # define inference
    serialized_tf_example = tf.placeholder(tf.string,shape=[10])
    feature_configs = {
        "y": tf.FixedLenFeature([], tf.float32),
        "feat_ids": tf.FixedLenFeature([FLAGS.common_field_size], tf.int64),       # for_change common field size ??
        "feat_vals": tf.FixedLenFeature([FLAGS.common_field_size], tf.float32),
    ...
    inference_features  = tf.parse_example(serialized_tf_example, feature_configs)
    inference_labels = inference_features.pop('y')
    #labels = tf.placeholder(tf.float32,shape=[None,1])
    inference_deep = dnn_layer(inference_features)
        ...
        if FLAGS.task_type == "train":
            ...
            try:
                while not coord.should_stop():
                    ...
                    _,step = session.run([optimizer_op,global_step])
                    #if step == (num_steps - 1):
            ...
            infer_set = tf.data.TFRecordDataset(te_files)
            infer_set = infer_set.batch(10)
            iter_infer = infer_set.make_one_shot_iterator()
            serialized_example = iter_infer.get_next()
            coord = tf.train.Coordinator()
            threads = tf.train.start_queue_runners(coord=coord, sess=session)
            try:
                while not coord.should_stop():
                    ...
                    infer_vec = session.run([inference_deep],feed_dict={serialized_tf_example:ser_list})
                    for vec in infer_vec[0]:
                        print vec
            except tf.errors.OutOfRangeError:
                print("Data is cover!")
            coord.request_stop()
            ...
            inputs = {
                        'input': tf.saved_model.utils.build_tensor_info(serialized_tf_example)
                    }
            output = {'output': tf.saved_model.utils.build_tensor_info(inference_deep)}
